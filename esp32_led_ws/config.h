...
// ─────────────────────────────────────────────
#define WIFI_SSID "#ILLAR"
#define WIFI_PASSWORD "12345678"
#define HOSTNAME "led-node-1" // compile-time only (mDNS hostname)
#define SHELF_ID_DEFAULT                                                       \
  1 // default shelf number — overridable at runtime via POST /config
#define FIRMWARE_VERSION "1.0.0" // bump on every flash to production hardware

// Java middleware (Lagerverwaltung, Tomcat) — this device connects out to it as
// a WebSocket client (/ws/storage) in addition to running its own local server
// below.
#define MW_HOST "192.168.1.10"
#define MW_PORT 8080
#define MW_PATH "/Lagerverwaltung/ws/storage"
...
// Device identity — runtime-configurable via POST /config, stored in
// config.json
